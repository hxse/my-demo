...
const englishDb = new Dexie("englishDb");
englishDb.version(1).stores({
  text: "++id,text",
});
const chineseDb = new Dexie("chineseDb");
...
async function addFileDb(name, text) {
  ...
  for (let i of text) {
    switch (name) {
      case 'english':
        await englishDb.text.add({ text: i });
        break;
      case 'chinese':
        await chineseDb.text.add({ text: i });
        break;
    }
  }
  console.log(englishDb.text,123424);
}
function App() {
