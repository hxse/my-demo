...
function App() {
  ...
  function contentPage(id, way) {
    ...
    for (let span of contentDiv.querySelectorAll("span")) {
      ...
      if (spanArr[spanArr.length].name == "content" && spanBottom - contentBottom <= 1) {
        consoele.log("捕捉误差 content", span);
      }
      if (spanArr[spanArr.length].name == "_down" && spanBottom - contentBottom > 1) {
        consoele.log("捕捉误差 _down", span);
      }
    }
