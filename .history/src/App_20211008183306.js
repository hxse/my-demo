  ...
  function contentPage(id, way) {
    ...
    let contentClass = contentSpan.reduce(function (pre, current, index) {
      ...
      if (index == 0) {
        ...
        console.log(index, span.offsetTop, pre[pre.length - 1][pre[pre.length - 1].length-1].offsetTop);
      if (span.offsetTop == pre[pre.length - 1][pre[pre.length - 1].length - 1].offsetTop.offsetTop) {
