...
function getData(language, num) {
  fetch(`http://127.0.0.1:8000/ATG/${language}/${num}`)
    .then((res) => res.json())
    .then(
      (result) => {
        console.log(result);
      },
      // 注意：使用 catch 去捕获异常会掩盖掉组件本身可能产生的 bug, 所以使用这个
      (error) => {
...
function App() {
  ...
  useEffect(() => {
    fetch("http://127.0.0.1:8000/config/get/ATG")
      .then((res) => res.json())
      .then(
        (result) => {
          ...
          console.log(result.history[1][0][0][0]);
        },
        // 注意：使用 catch 去捕获异常会掩盖掉组件本身可能产生的 bug, 所以使用这个
        (error) => {
          console.log("报错", error);
    ...
    setChapter((i)=>getData("chinese", i))
