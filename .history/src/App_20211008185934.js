...
function App() {
  ...
  function getRows(spans) {
    ...
    return spans.reduce(function (pre, current, index) {
      let span = current.data;
      console.log(index, span.offsetTop);
      if (index == 0) {
        return [[span]];
      }
      if (span.offsetTop == pre[pre.length - 1][pre[pre.length - 1].length - 1].offsetTop) {
        pre[pre.length - 1].push(span);
        return pre;
      } else {
        return [...pre, [span]];
      }
  ...
  function contentPage(id, way) {
    ...
    if (way == "down") {
      ...
      spanContentEndRow.forEach((i) => {
        ...
        i.style.textDecorationThickness = "0.02em";
        i.style.textDecorationStyle = "solid";
      });
    } else {
      ...
      spanContentStartRow.forEach((i) => {
        i.style.textDecoration = "underline";
        i.style.textDecorationThickness = "0.02em";
        i.style.textDecorationStyle = "dashed";
      });
    }
  }
    ...
    <div className="App">
      <div className="container" style={{ height: use100vh() * 0.95 }}>
        ...
        <div id="reader" className="content">
          ...
          <div id="content2"> {data ? data.map((i) => <span>{i}</span>) : undefined}</div>
          <div className="hr-wrapper">
