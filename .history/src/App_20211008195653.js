...
function App() {
  ...
  function contentPage(id, way) {
    ...
    for (let span of contentDiv.querySelectorAll("span")) {
      ...
      if (spanArr[spanArr.length-1].name == "content" &&spanTop < contentBottom&& spanBottom - contentBottom >= 1) {
        console.log("捕捉误差 content", span,spanBottom,contentBottom);
      }
      if (spanArr[spanArr.length-1].name == "_down" && spanBottom - contentBottom < 1) {
        console.log("捕捉误差 _down", span, spanBottom, contentBottom);
      }
    }
