...
function App() {
  ...
  function contentPage(id, way) {
    ...
    for (let span of contentDiv.querySelectorAll("span")) {
      ...
      if (spanTop < contentTop && spanBottom < contentTop) {//上方
        spanArr.push({ name: "up", data: span ,idx:spanArr.length});
      }
      if (spanTop < contentTop && spanBottom > contentTop) {//上方临界
        spanArr.push({ name: "_up", data: span, idx: spanArr.length });
      }
      if (spanTop >= contentTop && spanBottom <= contentBottom) {//正文
        spanArr.push({ name: "content", data: span, idx: spanArr.length });
      }
      if (spanTop < contentBottom && spanBottom > contentBottom) {//下方临界
        spanArr.push({ name: "_down", data: span, idx: spanArr.length });
        console.log("下方临界", spanTop, spanBottom, contentBottom, spanBottom-contentBottom);
      }
      if (spanTop > contentBottom && spanBottom > contentBottom) {//下方
    ...
    if (way == "down") {
      ...
      if (spanContentEnd.idx == spanArr.length - 2) {//最后一行
