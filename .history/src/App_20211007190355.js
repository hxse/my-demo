...
setInterval(() => {
  if (gamepadIndex !== undefined) {
    ...
    let pressNum = myGamepad.axes.filter((item, idx) => idx == 1 || idx == -1);
    // if (bingjiNum > 1) {
    //   //并击
    //   bingji = true;
    // }
    if (pressNum == 0) {
        console.log(2345, _axes);

      if (_axes.filter((i) => i == true).length > 0) {
        console.log(1234,_axes)
        for (let i in _axes) {
          _axes[i] = false;
        }
        // bingji = false;
