...
function App() {
  ...
  function contentPage(id, way) {
    ...
    for (let span of contentDiv.querySelectorAll("span")) {
      // console.log(1234, span.offsetTop,span.offsetHeight, contentDiv.offsetHeight);
      if (span.offsetTop < span.scrollTop) {
        console.log("上方", span.offsetTop,  contentDiv.offsetHeight, span);
      }
      if (span.offsetTop >= span.scrollTop && span.offsetTop <=span.scrollTop+ contentDiv.offsetHeight) {
        console.log("中央", span.offsetTop, contentDiv.offsetHeight, span);
      }
      if (span.offsetTop > span.scrollTop + contentDiv.offsetHeight) {
        console.log("下方", span.offsetTop, contentDiv.offsetHeight, span);
      }
    }
    if (way == "down") {
