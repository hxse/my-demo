  ...
  function contentPage(id, way) {
    ...
    for (let span of contentDiv.querySelectorAll("span")) {
      ...
      if (spanTop < contentTop && spanBottom > contentTop) {
        console.log("上临界", span);
      }
      if (spanTop <= contentTop && spanBottom < contentTop && spanTop > contentBottom && spanBottom > contentBottom) {
        console.log("中央", span);
      }
      if (spanTop < contentBottom && spanBottom > contentBottom) {
        console.log("下临界", span);
      }
    }
  }
  function contentPage3(id, way) {
    let contentDiv = document.querySelector(id);
    let spanPosition = [];
    let _spanUp;
    let spanContentStart;
    let spanContentEnd;
    let _spanDown;
    let spanDown;
    for (let span of contentDiv.querySelectorAll("span")) {
      if (span.offsetTop + span.offsetHeight < contentDiv.scrollTop) {
        // console.log("上方", span.offsetTop, contentDiv.scrollTop, contentDiv.offsetHeight, span);
      }
      if (span.offsetTop < contentDiv.scrollTop && span.offsetTop + span.offsetHeight > contentDiv.scrollTop) {
        console.log("上方临界", span.offsetTop, contentDiv.scrollTop, contentDiv.offsetHeight, span);
        _spanUp = span;
      }
      if (
        span.offsetTop >= contentDiv.scrollTop &&
      ...
      if (
        ...
        console.log("下方临界", span.offsetHeight, span.offsetTop, contentDiv.scrollTop, contentDiv.offsetHeight, span);

        let contentBottom = contentDiv.getBoundingClientRect().bottom;
        let contentTop = contentDiv.getBoundingClientRect().top;
        let spanBottom = span.getBoundingClientRect().bottom;
        let spanTop = span.getBoundingClientRect().top;
        console.log(spanTop, spanBottom, spanTop > contentBottom, spanBottom > contentBottom);
        _spanDown = span;
      }
      if (span.offsetTop > contentDiv.scrollTop + contentDiv.offsetHeight) {
